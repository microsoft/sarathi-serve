...
class DataSeries:
    ...
    def merge(self, other: "DataSeries"):
        ...
        assert self._x_name == other._x_name
        assert self._y_name == other._y_name

        self._data_series.extend(other._data_series)

        # sort by y
        self._data_series = sorted(self._data_series, key=lambda x: x[0])
        self._last_data_y = self._data_series[-1][1]
    ...
    def _to_df(self):
        return pd.DataFrame(self._data_series,
                            columns=[self._x_name, self._y_name])
    ...
    def plot_histogram(self, path: str, plot_name: str) -> None:
        ...
        histogram_df = df[self._y_name].value_counts(bins=25,
    ...
    def plot_differential(self, path: str, plot_name: str) -> None:
        ...
        df = df.groupby(self._x_name).max().reset_index().sort_values(
            self._x_name)

        # compute differential
        df[differential_col_name] = df[self._y_name].diff() / df[
            self._x_name].diff()
