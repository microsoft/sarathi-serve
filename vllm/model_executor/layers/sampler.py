...
def _prune_hidden_states(
    ...
    for i, seq_group in enumerate(input_metadata.seq_groups):
        ...
        if i < input_metadata.num_prompts:
            ...
            prompt_len = input_metadata.current_prompt_chunk_lens[i]
...
def _random_sample(
    ...
    # make sure that probs don't contain any NaNs, negative or infinite values
    # replace nan's with 0.0
    # Identify the inf and nan values
    # Replace the inf and nan values with the computed mean
    probs[~torch.isfinite(probs)] = 0.0
    probs = probs.clamp(min=0.0)
    # replace zeros with small value to avoid zero probabilities
    probs[probs == 0.0] = 1e-6
