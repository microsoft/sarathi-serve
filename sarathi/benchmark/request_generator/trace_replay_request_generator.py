...
class TraceReplayRequestGenerator(BaseRequestGenerator):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._trace_df = self._trace_df[self._trace_df["Date"] == self._config.
                                        trace_request_generator_date]
        ...
        self._trace_df["PromptTokenCount"] = self._trace_df[
            "PromptTokenCount"].astype(int)
        self._trace_df["CompletionTokenCount"] = self._trace_df[
            "CompletionTokenCount"].astype(int)

        # make sure that there is at least one prefill and decode token
        self._trace_df["PromptTokenCount"] = self._trace_df[
            "PromptTokenCount"].clip(lower=1)
        self._trace_df["CompletionTokenCount"] = self._trace_df[
            "CompletionTokenCount"].clip(lower=1)

        # make sure the total does not exceed the max tokens, adjust the prefill tokens if needed
        total_tokens = (self._trace_df["PromptTokenCount"] +
                        self._trace_df["CompletionTokenCount"])
        diff_tokens = total_tokens - self._config.trace_request_generator_max_tokens
        diff_tokens = diff_tokens.clip(lower=0)
        self._trace_df["PromptTokenCount"] = (
            self._trace_df["PromptTokenCount"] - diff_tokens)

        assert all(self._trace_df["PromptTokenCount"] +
                   self._trace_df["CompletionTokenCount"] <=
                   self._config.trace_request_generator_max_tokens)
        ...
        pd_ratio = (self._trace_df["PromptTokenCount"] /
                    self._trace_df["CompletionTokenCount"])
        logger.info(
