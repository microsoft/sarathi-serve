...
class TraceRequestLengthGenerator(BaseRequestLengthGenerator):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._trace_df["num_prefill_tokens"] = self._trace_df[
            "num_prefill_tokens"].astype(int)
        self._trace_df["num_decode_tokens"] = self._trace_df[
            "num_decode_tokens"].astype(int)

        # make sure the total does not exceed the max tokens, adjust the prefill tokens if needed
        total_tokens = (self._trace_df["num_prefill_tokens"] +
                        self._trace_df["num_decode_tokens"])
        diff_tokens = total_tokens - self._config.trace_request_length_generator_max_tokens
        diff_tokens = diff_tokens.clip(lower=0)

        # dedcut the diff tokens from the prefill and decode tokens proportionally
        prefill_tokens_ratio = self._trace_df[
            "num_prefill_tokens"] / total_tokens
        decode_tokens_ratio = self._trace_df["num_decode_tokens"] / total_tokens

        self._trace_df["num_prefill_tokens"] -= (np.ceil(
            diff_tokens * prefill_tokens_ratio)).astype(int)

        self._trace_df["num_decode_tokens"] -= (np.ceil(
            diff_tokens * decode_tokens_ratio)).astype(int)

        # make sure that there is at least one prefill and decode token
        self._trace_df["num_prefill_tokens"] = self._trace_df[
            "num_prefill_tokens"].clip(lower=1)
        self._trace_df["num_decode_tokens"] = self._trace_df[
            "num_decode_tokens"].clip(lower=1)

        assert all(self._trace_df["num_prefill_tokens"] +
                   self._trace_df["num_decode_tokens"] <=
        ...
        pd_ratio = (self._trace_df["num_prefill_tokens"] /
                    self._trace_df["num_decode_tokens"])
        logger.info(
        ...
        self._trace_df = self._trace_df.sample(frac=1,
