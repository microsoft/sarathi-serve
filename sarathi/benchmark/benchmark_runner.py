...
class BenchmarkRunner:
    ...
    def __init__(self,
                 replica_id: int,
                 config: Config,
                 replica_resource_mapping: ResourceMapping = []) -> None:
        ...
        self._llm_engine = LLMEngine.from_engine_args(
            ...
            enable_dynamic_chunking_schedule=self._config.
            sarathi_scheduler_enable_dynamic_chunking_schedule,
            ...
            chunk_schedule_max_tokens=self._config.
            sarathi_scheduler_chunk_schedule_max_tokens,
            chunk_schedule_stages=self._config.
            sarathi_scheduler_chunk_schedule_stages,
            ...
            keep_individual_batch_metrics=self._config.
            metrics_store_keep_individual_batch_metrics,
            # engine config
            trust_remote_code=True,
        )

    def _get_input_params(self, request: Request,
                          first_request_time: float) -> SamplingParams:
...
class BenchmarkRunnerLauncher:
    ...
    def __init__(self, config: Config) -> None:
        ...
        if self._is_multi_replica:
            ...
            self._runner = BenchmarkRunner(0, self._config,
    ...
    def _get_replica_resource_mapping(self) -> ReplicaResourceMapping:
        ...
        num_replicas = self._config.cluster_num_replicas
        num_gpus_per_replica = self._config.model_tensor_parallel_degree * self._config.model_pipeline_parallel_degree

        assert num_gpus >= num_replicas * num_gpus_per_replica, \
    ...
    def _create_runners(self):
        assert self._config.model_tensor_parallel_degree > 1 or self._config.model_pipeline_parallel_degree > 1
        ...
        for replica_id in range(self._config.cluster_num_replicas):
            runners.append(
                runner_class.options(resources={
                    replica_resource_mapping[str(replica_id)][0][0]:
                    0.01,
                }, ).remote(replica_id, self._config,
                            replica_resource_mapping[str(replica_id)]))
