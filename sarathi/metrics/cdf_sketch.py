...
class CDFSketch:
            ...
            wandb.log(
                {
                    ...
                    self.sketch.get_quantile_value(0.25),
                    f"{plot_name}_median":
                    self.sketch.get_quantile_value(0.5),
                    f"{plot_name}_75th_percentile":
                    self.sketch.get_quantile_value(0.75),
                    f"{plot_name}_95th_percentile":
                    self.sketch.get_quantile_value(0.95),
                    f"{plot_name}_99th_percentile":
                    self.sketch.get_quantile_value(0.99),
                    f"{plot_name}_99.9th_percentile":
                    self.sketch.get_quantile_value(0.999),
                    f"{plot_name}_count":
                    self.sketch.count,
                    f"{plot_name}_sum":
                    self.sketch.sum,
    ...
    def to_df(self) -> pd.DataFrame:
        ...
        df = pd.DataFrame({
            "cdf": quantiles,
            self.metric_name: quantile_values
        })
