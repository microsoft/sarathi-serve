...
class APIServerConfig(BaseEndpointConfig):
    log_level: str = field(
        default="debug",
        metadata={"help": "Logging level for the API server."}
    )
    host: str = field(
        default="localhost",
    ...
    port: int = field(
        default=8000,
        metadata={"help": "Port number to run the server on."}
    )
    ...
    ssl_cert_reqs: int = field(
        ...
        metadata={"help": "SSL certificate requirements (CERT_NONE, CERT_OPTIONAL, or CERT_REQUIRED)."}
    )
    server_root_path: Optional[str] = field(
        default=None,
        metadata={"help": "Root path for the server (useful for reverse proxy setups)."}
    )
